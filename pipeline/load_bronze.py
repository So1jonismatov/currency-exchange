...
from extract import fetch_currencies, fetch_rates
...
def load_to_bronze(base, targets, date, end_date=None):
    ...
    try:
        ...
        if rates_data:
            ...
            logger.info(
                f"Successfully loaded records into bronze.raw_rates for {ref_date}"
            )
        ...
        if currencies_data:
            ...
            cur.execute(
                query_curr,
                (datetime.now().strftime("%Y-%m-%d"), json.dumps(currencies_data)),
            )
